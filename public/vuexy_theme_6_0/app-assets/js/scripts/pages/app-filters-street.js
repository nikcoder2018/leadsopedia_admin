...
$(async function() {
    ...
    var dtCurrentTable = $('.current-list-table'),
        ...
        API_URL = '/api/filters/street/data',
        URL = '/filters',
        ...
        var dtNew = dtNewTable.DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row, meta) {
                    let elAction = '';
                    if (can_edit_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-edit text-info" href="javascript:void(0);" data-name="${row.street}" data-toggle="tooltip" data-placement="top" title="Edit">${feather.icons['edit'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    if (can_create_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-addto text-primary" href="javascript:void(0);" data-name="${row.street}" data-toggle="tooltip" data-placement="top" title="Add to Filters">${feather.icons['plus'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    return (
                        `<div class="d-flex align-items-center col-actions">
                            ${elAction}
    ...
    dtCurrentTable.on('click', '.btn-delete', async function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const deleteData = await $.get(`/filter/street/${id}/delete`);
