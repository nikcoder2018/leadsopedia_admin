...
$(function() {
    ...
    async function loadInbox() {
        tab = 'inbox';
        emailUserList.html(await $.get('/messages/inbox'));
    }
    async function loadSent() {
        tab = 'sent';
        emailUserList.html(await $.get('/messages/sent'));
    }
    async function loadTrash() {
        tab = 'trash';
        emailUserList.html(await $.get('/messages/trash'));
    }
    loadInbox();

    sidebarMenuList.on('click', '.list-group-item', function() {
        sidebarMenuList.find('.list-group-item').removeClass('active');
    ...
    if (compose.length) {
        compose.on('click', function() {
            ...
            // all input forms
            $('.compose-form input[type=text]').val('');
    ...
    emailUserList.on('click', 'li', async function(e) {
        let data = $(this).data();
        let details = await $.get(`/messages/${data.id}/read`);
        emailDetails.html(details);
        if (tab == 'inbox')
    ...
    emailDetails.on('click', '.go-back', function(e) {
        e.stopPropagation();
        emailDetails.removeClass('show');
    });

    emailDetails.on('click', '.email-del', function(e) {
        e.stopPropagation();
        $.get('/messages/delete', { ids: $(this).data('id') }).then(() => {
            emailDetails.removeClass('show');
            toastr['error']('You have removed message.', 'Message Deleted!', {
                closeButton: true,
                tapToDismiss: false
            });
            loadInbox();
    ...
    emailDetails.on('click', '.email-next', async function(e) {
        let data = $(this).data();
        let details = await $.get(`/messages/${data.id}/next`);
        if (details != '') {
            emailDetails.html(details);
            feather.replace();
        }
    });
    emailDetails.on('click', '.email-prev', async function(e) {
        let data = $(this).data();
        let details = await $.get(`/messages/${data.id}/prev`);
        if (details != '') {
            emailDetails.html(details);
            feather.replace();
        }
    ...
    $(document).on('click', '.email-app-list .selectAll input', function() {
        if ($(this).is(':checked')) {
            ...
            emailUserList.find('.custom-checkbox input').prop('checked', '').closest('.media').removeClass('selected-row-bg');
        }
    });

    // Delete selected Mail from list
    if (mailDelete.length) {
        mailDelete.on('click', async function() {
            if (emailUserList.find('.custom-checkbox input:checked').length) {
                ...
                let ids = [];
                $.each(emailUserList.find('.custom-checkbox input:checked'), function() {
                    ids.push($(this).val());
                })
                emailUserList.find('.custom-checkbox input:checked').closest('.media').remove();
                emailUserList.find('.custom-checkbox input').prop('checked', '');

                switch (tab) {
                    case 'inbox':
                        await $.get('/messages/delete', { ids: ids.join(',') });
                        break;
                    case 'sent':
                        await $.get('/messages/delete-sent', { ids: ids.join(',') });
                        break;
                    case 'trash':
                        await $.get('/messages/delete-force', { ids: ids.join(',') });
                        break;
                }
    ...
    composeModal.on('submit', 'form', function(e) {
        e.preventDefault();
        $.ajax({
            url: $(this).attr('action'),
            type: 'POST',
            data: $(this).serialize() + '&message=' + emailEditor.root.innerHTML,
    ...
    emailDetails.on('click', '.reply-message', function() {
        let email = $(this).data('email');
        compose.trigger('click');
        composeModal.find('input[name=to]').val(email);
    })
