...
$(async function() {
    ...
    var dtCurrentTable = $('.current-list-table'),
        ...
        API_URL = '/api/filters/titles/data',
        URL = '/filters',
        API_TOKEN = $('[name=api-token]').attr('content'),
        addto_group_modal = $('#addto-group-modal'),
        ...
        var dtCurrent = dtCurrentTable.DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row, meta) {
                    ...
                    elAction += `<a class="mr-1 btn-addtogroup text-primary" href="javascript:void(0);" data-id="${row._id}" title="Add to Group">${feather.icons['plus'].toSvg({ class: 'font-medium-2' })}</a>`;
        ...
        var dtNew = dtNewTable.DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row, meta) {
                    let elAction = '';
                    if (can_edit_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-edit text-info" href="javascript:void(0);" data-name="${row.title}" data-toggle="tooltip" data-placement="top" title="Edit">${feather.icons['edit'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    if (can_create_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-addto text-primary" href="javascript:void(0);" data-name="${row.title}" data-toggle="tooltip" data-placement="top" title="Add to Filters">${feather.icons['plus'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    return (
                        `<div class="d-flex align-items-center col-actions">
                            ${elAction}
    ...
    dtCurrentTable.on('click', '.btn-addtogroup', async function() {
        let id = $(this).data('id');
    ...
    addto_group_modal.on('submit', 'form', function(e) {
        e.preventDefault();
        var form = this;
        $.ajax({
            url: $(this).attr('action'),
