...
$(async function() {
    ...
    var dtCurrentTable = $('.current-list-table'),
        ...
        API_URL = '/api/filters/industry/data',
        URL = '/filters',
        API_TOKEN = $('[name=api-token]').attr('content'),
        addto_category_modal = $('#addto-category-modal'),
        ...
        var dtCurrent = dtCurrentTable.DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row, meta) {
                    ...
                    elAction += `<a class="mr-1 btn-addtocategory text-primary" href="javascript:void(0);" data-id="${row._id}" title="Add to Category">${feather.icons['plus'].toSvg({ class: 'font-medium-2' })}</a>`;
        ...
        var dtNew = dtNewTable.DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, row, meta) {
                    let elAction = '';
                    if (can_edit_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-edit text-info" href="javascript:void(0);" data-name="${row.industry}" data-toggle="tooltip" data-placement="top" title="Edit">${feather.icons['edit'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    if (can_create_filters || has_full_access) {
                        elAction += `<a class="mr-1 btn-addto text-primary" href="javascript:void(0);" data-name="${row.industry}" data-toggle="tooltip" data-placement="top" title="Add to Filters">${feather.icons['plus'].toSvg({ class: 'font-medium-2' })}</a>`;
                    }
                    return (
                        `<div class="d-flex align-items-center col-actions">
                            ${elAction}
    ...
    dtCurrentTable.on('click', '.btn-edit', async function() {
        ...
        const filter = await $.get('/filter/industry/' + id + '/edit', { type: 'current' });
    ...
    dtCurrentTable.on('click', '.btn-delete', async function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const deleteData = await $.get(`/filter/title/${id}/delete`);
    ...
    dtCurrentTable.on('click', '.btn-addtocategory', async function() {
        let id = $(this).data('id');
        addto_category_modal.find('input[name=id]').val(id);
        addto_category_modal.modal('show');
    });
    ...
    addto_category_modal.on('submit', 'form', function(e) {
        e.preventDefault();
        var form = this;
        $.ajax({
            url: $(this).attr('action'),
            ...
            success: function(resp) {
                if (resp.success) {
                    addto_category_modal.modal('hide');
