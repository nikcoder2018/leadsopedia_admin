...
$(async function() {
    ...
    var dtTransactionsTable = $('.transactions-list-table'),
        ...
        API_URL = '/api/transactions/archivelist',
        URL = '/transactions',
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (can_restore_transaction || has_full_access) {
                            elAction += `<a class="mr-1 text-success btn-restore" href="javascript:void(0);" data-id="${row.id}" data-toggle="tooltip" data-placement="top" title="Restore">${feather.icons['rewind'].toSvg({ class: 'font-medium-2' })}</a>`;
                        }
                        if (can_delete_transaction || has_full_access) {
    ...
    $(dtTransactionsTable).on('click', '.btn-restore', function() {
        let id = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                const restoreData = await $.get(`${URL}/${id}/restore`);
                if (restoreData.success) {
                    toastr['success'](restoreData.msg, 'Restored!', {
                        closeButton: true,
                        tapToDismiss: false,
                        rtl: isRtl
                    });
    ...
    $(dtTransactionsTable).on('click', '.btn-delete', function() {
        let id = $(this).data('id');
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                const restoreData = await $.get(`${URL}/${id}/delete`);
                if (restoreData.success) {
                    toastr['success'](restoreData.msg, 'Deleted!', {
                        closeButton: true,
                        tapToDismiss: false,
                        rtl: isRtl
                    });
